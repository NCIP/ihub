...
/**
 * This class was generated by Apache CXF 2.4.1 2012-04-12T15:17:59.068-04:00
...
public class CaXchangeRequestService extends AcceptMessage {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(CaXchangeRequestService.class.getName());
...
//						.getXmlSchemaDefinition().toString());
		...
		rm.setExternalIdentifier(parameters.getMetadata()
				.getExternalIdentifier());
		// TODO : need to find a way to get this identifier from MC or set this
		// id from here
		rm.setCaXchangeIdentifier(parameters.getMetadata()
		...
		try {
			...
			if ((mcResponse.indexOf("Error") > -1
					|| mcResponse.indexOf("Exception") > -1
					|| mcResponse.indexOf("ERROR") > -1 || mcResponse
					.indexOf("error") > -1)) {
				...
				Response response = new Response();
				response
						.setCaXchangeError(getErrorDetailsFromCaXchangeError(mcResponse));
				response.setResponseStatus(Statuses.FAILURE);
			...
			ErrorDetails errorDetails = new ErrorDetails();
			errorDetails.setErrorCode(String.valueOf(IntegrationError._1000
					.getErrorCode()));
			errorDetails.setErrorDescription(IntegrationError._1000
					.getMessage(null)
					+ ex.getMessage());
	...
	private String getCaXchangeRequestxml(final Message parameter) {
		try {
			...
			QName qname = new QName(
					"http://caXchange.nci.nih.gov/caxchangerequest",
					"caxchangerequest");
			JAXBElement<Message> message = new JAXBElement<Message>(qname,
	...
	private ErrorDetails getErrorDetailsFromCaXchangeError(String errorXml)
