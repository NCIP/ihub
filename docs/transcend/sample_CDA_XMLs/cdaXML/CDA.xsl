...
<!--
  ...
  Rivision History:  03/30/10 Jingdong Li updated.
  Rivision History:  08/25/10 Jingdong Li updated
  Rivision History:  09/17/10 Jingdong Li updated
  Rivision History:  01/05/10 Jingdong Li updated
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
				...
				<tbody>
					<xsl:for-each select="n1:author/n1:assignedAuthor">
						<tr>
							...
							<td width="80%">
								<xsl:choose>
									<xsl:when test="not(n1:assignedPerson/n1:name/@nullFlavor)">
										<xsl:call-template name="show-name">
											<xsl:with-param name="name"
												select="n1:assignedPerson/n1:name"/>
										</xsl:call-template>
										<xsl:if test="not(n1:representedOrganization/@nullFlavor)">
											<xsl:text>, </xsl:text>
											<xsl:call-template name="show-name">
												<xsl:with-param name="name"
				...
				<tbody>
					<tr>
						...
						<td width="80%">
							<xsl:choose>
								<xsl:when
									test="not(n1:custodian/n1:assignedCustodian/n1:representedCustodianOrganization/n1:name/@nullFlavor)">
									<xsl:call-template name="show-name">
										<xsl:with-param name="name"
							...
							<xsl:if test="$displayName">
								<tr>
									...
									<td width="80%" colspan="3">
										...
										<xsl:if
											test="string-length(n1:serviceEvent/n1:id/@extension)>0">
											<xsl:text> (ID: </xsl:text>
											<xsl:call-template name="show-id">
												<xsl:with-param name="id"
												select="n1:serviceEvent/n1:id"/>
											</xsl:call-template>
											<xsl:text>) </xsl:text>
										</xsl:if>
			...
			<tbody>
				<xsl:for-each select="/n1:ClinicalDocument/n1:recordTarget/n1:patientRole">
					<xsl:if test="not(n1:id/@nullFlavor)">
						...
						<xsl:if test="n1:patient/n1:raceCode | (n1:patient/n1:maritalStatusCode)">
							<tr>
								<td width="20%" bgcolor="#3399ff">
									<span class="td_label">
								...
								<td width="30%">
									<xsl:choose>
										<xsl:when test="n1:patient/n1:raceCode">
											...
											<xsl:for-each
												select="//n1:section/n1:entry/n1:observation[n1:code[@code='103579009'][@codeSystem='2.16.840.1.113883.6.96']]/n1:value">
												<br/>
												<xsl:call-template name="show-race-ethnicity"/>
											</xsl:for-each>
										</xsl:when>
								...
								<td>
									<xsl:choose>
										<xsl:when test="n1:patient/n1:maritalStatusCode">
   ...
   -->
	<xsl:template name="section">
		...
		<!--
		<xsl:for-each select="n1:component/n1:section">
			<xsl:if test="string-length(n1:title)>0">
 ...
 @mediaType='image/jpeg']">
	...
	<xsl:template name="show-gender">
		<xsl:choose>
			...
			<xsl:when test="string-length(n1:originalText)>0">
				<xsl:value-of select="n1:originalText"/>
			</xsl:when>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="show-contactInfo">
		...
		<xsl:choose>
			<xsl:when test="($contact/n1:addr[@nullFlavor]) and $contact/n1:telecom[@nullFlavor]">
	...
	<xsl:template name="show-time">
		...
		<xsl:choose>
			...
			<xsl:when test="$datetime/@nullFlavor">
				<xsl:variable name="timeNull">
					<xsl:call-template name="show-nullFlavor">
						<xsl:with-param name="nf" select="$datetime/@nullFlavor"/>
					</xsl:call-template>
				</xsl:variable>
				<xsl:call-template name="firstCharCaseUp">
					<xsl:with-param name="data" select="$timeNull"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:otherwise>
				<xsl:call-template name="formatDateTime">
