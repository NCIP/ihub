...
/**
 * This utility class resolve properties from any of following locations in the
 * same order
 * 
 * <pre>
 ...
 * ${user.home}/.integration/${project.name}/${propertyFile.name} project will
 * simply ignore that property from other property files.
 * 
...
public final class CommonsPropertyLoaderUtil {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(CommonsPropertyLoaderUtil.class);
	 ...
	 *            - property file name
	 * @return properties if it can resolve any of the property file, exceptions
	 *         if no property file is available
	 */
	public static Properties loadProperties(final String projectName,
			final String propertyFileName) {
		if (StringUtils.isEmpty(projectName)
				|| StringUtils.isEmpty(propertyFileName)) {
			throw new IllegalArgumentException(
					"Project name and property file name must not be null!");
		}

		final String userHomePathToScan = String.format(
				"user.home/.integration/%s/%s", projectName, propertyFileName);
		final String appHomePathToScan = String.format(
				"app.home/.integration/%s/%s", projectName, propertyFileName);
		...
		for (Map.Entry<Object, Object> entry : properties.entrySet()) {
			LOG.info(String.format("loaded property - %s:%s", entry.getKey(),
					entry.getValue()));
		}
		return properties;
	 ...
	 *            - environment variable (for ex : user.home, app.home etc)
	 * @param pathToScan
	 *            - the detailed path to scan
	 */
	private static void fillPropertiesFromFileSystem(Properties properties,
			final String environmentVariable, String pathToScan) {
		if (StringUtils.isBlank(System.getProperty(environmentVariable))) {
			LOG.info(String.format(
					"%s is not set. So skipping property lookup from %s",
					environmentVariable, environmentVariable));
		} else {
			final String detailedEnvironmentPath = StringUtils.replace(
					pathToScan, environmentVariable, System
	...
	private static void fillProperties(Resource resource, Properties properties) {
		try {
			if (resource.exists()) {
				LOG.info("merging properties form : "
				...
				LOG
						.info(String
								.format(
										"can not merge property from %s as resource does not exists.",
										resource.getDescription()));
			}
		} catch (IOException e) {
