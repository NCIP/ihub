...
/**
 * This is the client class for Specimen Flow. It provide operation like
 * CreateSpecimen, UpdateSpecimen, RollbackSpecimen
 * 
 * @author Rohit Gupta
 */
...
public class CaTissueSpecimenClient {
	...
	public CaTissueSpecimenClient(String loginName, String password)
			throws Exception {
		super();
		Thread.currentThread().setContextClassLoader(
				CaTissueSpecimenClient.class.getClassLoader());
	 ...
	 *             - Throws exception if Specimen already exist
	 */
	public String isSpecimensExist(String specimenListXMLStr)
			throws ApplicationException {

		LOG
				.debug("Inside isSpecimensExist...The Incoming XML for isSpecimensExist() is --> "
						+ specimenListXMLStr);
	...
	/**
	 * This method is used to fetch the Specimen(s) details for given specimen
	 * XMLString
	 * 
	 * @throws ApplicationException
	 */
	public String getExistingSpecimens(String specimenListXMLStr)
			throws ApplicationException {

		LOG
				.debug("Inside getExistingSpecimens...The Incoming XML for getExistingSpecimens() is --> "
						+ specimenListXMLStr);
	 ...
	 *            -- The XML string for creating the bio-specimen which may
	 ...
	 *            multiple specimens.
	 */
	public String updateSpecimens(String specimenListXMLStr)
			throws ApplicationException {

		LOG
				.debug("Inside CaTissueSpecimenClient... updateSpecimens()..The Incoming XML is --> "
						+ specimenListXMLStr);
	 ...
	 * flow
	 */
	public String rollbackCreatedSpecimens(String specimenListXMLStr)
			throws ApplicationException {

		LOG
				.debug("Inside CaTissueSpecimenClient... rollbackCreatedSpecimens()..The Incoming XML is --> "
						+ specimenListXMLStr);
	 ...
	 * @throws ApplicationException
	 */
	public String rollbackUpdatedSpecimens(String specimenListXMLStr)
			throws ApplicationException {

		LOG
				.debug("Inside CaTissueSpecimenClient... rollbackUpdatedSpecimens()..The Incoming XML is --> "
						+ specimenListXMLStr);
	...
	private Specimens parseSpecimenListXML(String specimenListXMLStr) {
		Specimens specimens = (Specimens) xStream.fromXML(new StringReader(
				specimenListXMLStr));
		return specimens;
	}
	 ...
	 * @param specimens
	 * @throws ApplicationException
	 *             if specimen already exist
	 */
	private String isSpecimensAlreadyExist(Specimens specimens)
			throws ApplicationException {
		List<SpecimenDetail> specimenDetailList = specimens
				.getSpecimenDetailList();
		Iterator<SpecimenDetail> specimenDetailItr = specimenDetailList
				.iterator();
		while (specimenDetailItr.hasNext()) {
			...
			if (existingSpecimen != null) {
				LOG
						.error("Specimen with the same LABEL already exists for Specimen "
								+ specimenLabel);
				throw new ApplicationException(
						"Specimen with the same LABEL already exists.");
			}
	...
	/**
	 * This method is used to fetch the existing specimen(s) for corresponding
	 * incoming specimen label. This will be used incase rollback is required.
	 * 
	 * @param incomingSpecimens
	 */
	private Specimens fetchExistingSpecimens(Specimens incomingSpecimens)
			throws ApplicationException {
		List<Specimen> existingSpecimenList = new ArrayList<Specimen>();
		List<SpecimenDetail> specimenDetailList = incomingSpecimens
				.getSpecimenDetailList();
		Iterator<SpecimenDetail> specimenDetailItr = specimenDetailList
				.iterator();
		while (specimenDetailItr.hasNext()) {
			SpecimenDetail specimenDetail = specimenDetailItr.next();
			Specimen existingSpecimen = getExistingSpecimen(specimenDetail
					.getSpecimen().getLabel());
			if (existingSpecimen == null) {
				LOG.error("Specimen for given LABEL "
						+ specimenDetail.getSpecimen().getLabel()
						+ " doesn't exist.");
				throw new ApplicationException(
			...
			if (isUpdateSpecimenRequestDataValid(specimenDetail,
					existingSpecimen)) {
	 ...
	 *            to be created
	 */
	private void performCreateSpecimens(Specimens specimens)
			throws ApplicationException {
		...
		for (specimenDetailItr = specimenDetailList.iterator(); specimenDetailItr
				.hasNext();) {
			...
			if (scgFound == false) {
				// throw exception
				LOG
						.error("Specimen Collection Group was not found in caTissue for Label "
								+ specimen.getLabel());
				throw new ApplicationException(
						"Specimen Collection Group not found in caTissue");
			}
			...
			try {
				...
				LOG.error("CreateSpecimen Failed for Label"
						+ specimen.getLabel() + " and exception is "
						+ e.getCause());
				throw new ApplicationException(
						"CreateSpecimen Failed for Label"
								+ specimenDetail.getSpecimen().getLabel()
	...
	private Specimen createSpecimen(Specimen specimen)
			throws ApplicationException {
	 ...
	 * @throws Exception
	 */
	private List<Specimen> performUpdateSpecimens(Specimens specimens)
			throws ApplicationException {
		...
		try {
			for (specimenDetailItr = specimenDetailList.iterator(); specimenDetailItr
				...
				Specimen existingSpecimen = getExistingSpecimen(incomingSpecimen
						.getLabel());
				incomingSpecimen.setId(existingSpecimen.getId());
				incomingSpecimen.setSpecimenCollectionGroup(existingSpecimen
						.getSpecimenCollectionGroup());// Specimen Collection
														// Group is required.
			...
			LOG.error("UpdateSpecimen Failed for Label"
					+ specimenDetail.getSpecimen().getLabel()
					+ " and exception is " + e.getCause());
			throw new ApplicationException("UpdateSpecimen Failed for Label"
					+ specimenDetail.getSpecimen().getLabel()
					+ " and exception is " + e.getCause());
		}
	...
	/**
	 * This method is used to check if the updateSpecimen request had valid data
	 * by comparing the values of incoming specimen and existing specimen (doing
	 * check only for CP, CPE & SC).
	 * 
	 * @return
	 */
	private boolean isUpdateSpecimenRequestDataValid(
			SpecimenDetail inSpecimenDetail, Specimen existingSpecimen)
			throws ApplicationException {
		boolean hasValidData = true;
		String inCPE = inSpecimenDetail.getCollectionProtocolEvent();
		String existCPE = existingSpecimen.getSpecimenCollectionGroup()
				.getCollectionProtocolEvent().getCollectionPointLabel();
		String inCP = inSpecimenDetail.getCollectionProtocol().getTitle();
		String existCP = existingSpecimen.getSpecimenCollectionGroup()
				.getCollectionProtocolEvent().getCollectionProtocol()
				.getTitle();
		...
		if (!inCPE.equals(existCPE)) {
			hasValidData = false;
			LOG
					.error("UpdateSpecimen Request Failed for Label"
	...
	private void performRollbackUpdatedSpecimens(Specimens specimens)
		...
		while (specimenDetailItr.hasNext()) {
			...
			try {
				...
				LOG
						.error("Exception occured during Rollback of UpdateSpecimen "
								+ existingSpecimen.getLabel()
								+ " and exception is " + e.getCause());
				throw new ApplicationException(
						"Exception occured during Rollback of UpdateSpecimen "
								+ existingSpecimen.getLabel()
								+ " and exception is " + e.getCause());
			}
		}
	...
	private Specimens copyFromExistingSpecimen(
			List<Specimen> existingSpecimenList) {

		Specimens existingSpecimens = new Specimens();
		Iterator<Specimen> specimenItr = null;
		for (specimenItr = existingSpecimenList.iterator(); specimenItr
				.hasNext();) {
			...
			} else if (FLUID.equalsIgnoreCase(existingSpecimen
					.getSpecimenClass())) {
			...
			specimen.setPathologicalStatus(existingSpecimen
			...
			chars.setTissueSide(existingSpecimen.getSpecimenCharacteristics()
					.getTissueSide());
			chars.setTissueSite(existingSpecimen.getSpecimenCharacteristics()
					.getTissueSite());
			specimen.setSpecimenCharacteristics(chars);
			specimen.getSpecimenCharacteristics().setId(
			...
			specimen
					.setCollectionStatus(existingSpecimen.getCollectionStatus());
	...
	private void performRollbackCreatedSpecimens(Specimens specimens)
		...
		while (specimenDetailItr.hasNext()) {
			...
			try {
				...
				LOG
						.error("Exception occured during Rollback of CreateSpecimen "
								+ specimenLabel
								+ " and exception is "
								+ e.getCause());
				throw new ApplicationException(
						"Exception occured during Rollback of CreateSpecimen "
								+ specimenLabel + " and exception is "
								+ e.getCause());
			}
		}
	}
	 ...
	 * @throws ApplicationException
	 */
	private void softDeleteSpecimen(Specimen existingSpecimen)
			throws ApplicationException {
	 ...
	 * 
	 */
	private Specimen updateSpecimenLabel(Specimen specimen)
			throws ApplicationException {
		specimen.setLabel("DELETED_" + specimen.getLabel() + "_"
				+ getCurrentDateTime());
		specimen.setBarcode("DELETED_" + specimen.getBarcode() + "_"
				+ getCurrentDateTime());
	...
	private List<SpecimenCollectionGroup> getSpecimenCollectionGroupList(
			SpecimenDetail specimenDetail) throws ApplicationException {
