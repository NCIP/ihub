...
public class AcceptSourceFaultTest {
	...
	private String getCaCISFaultxml(final CaCISFault parameter) {
		try {
			...
			JAXBElement<CaCISFault> cfj = new JAXBElement<CaCISFault>(
					_CaCISFault_QNAME, CaCISFault.class, parameter);
	...
	private CaCISFault getCaCISFaultFromXml(String faultXML)
			throws JAXBException {
		StreamSource ss = new StreamSource(new StringReader(faultXML));
		return ((JAXBElement<CaCISFault>) getUnmarshaller().unmarshal(ss,
				CaCISFault.class)).getValue();
	}
